...
const BottomNav = () => {
  ...
  return (
    <div id='bottom-nav' className={theme === 'dark' ? 'dark' : ''}>
      ...
      <IconButton
        className={
          currentLocation === 'circuits' ? 'nav-btn active' : 'nav-btn'
        }
        onClick={() => handleClick(showHome || showGenerator ? 'circuits' : '')}
      >
        <p className={isIOS || isSafari ? '' : 'noto-color-emoji-regular'}>
          💻
        </p>
      ...
      <IconButton
        className={currentLocation === 'notes' ? 'nav-btn active' : 'nav-btn'}
